...
class Geodesics0(object):
    ...
    @property
    def conds(self):
        return self.gdss.keys()
        
    @property    
    def ts(self):
        return Series([gds.t for gds in self.gdss], index=self.conds)
    
    @property
    def stops(self):
        return Series([gds.stop for gds in self.gdss], index=self.conds)
    
    @property
    def ncalls(self):
        return Series([gds.ncall for gds in self.gdss], index=self.conds)
    
    @property
    def v0s(self):
        return DF([gds.v0 for gds in self.gdss], columns=self.pids)
    # FIXME ***: I've removed pids as an attribute
    
            
    def get(self, attrname):
        return Series([getattr(gds, attrname) for gds in self.gdss], 
                      index=self.conds)
    
    
    # Plan: remove the following two funcs; duplicates of butil.Series methods. 
    def map(self, f):
        """Name 'apply' may be better; 'map' implies that something is 
        coming out, like a function; but 'apply' is merely an operation,
        which can but doesn't have to have something coming out...
        """
        return Series([f(gds) for gds in self.gdss], index=self.conds)
    
    
    def filter(self, f_key=None, f_val=None):
        """
        """
        return Geodesics(gdss=self.gdss.filt(f_key=f_key, f_val=f_val)) 
        
            
    def get_eigvs(self, idx_t=-1, idx_eigv=-1):
        """FIXME: Shouls call it eigvec
        
        Input:
            idx_t: int, the index of time (not the actual time)
            idx_eigv: int, the index of eigenvector
        """
        return DF([np.linalg.svd(gds.Df(gds.ptraj.iloc[idx_t]))[-1][idx_eigv] 
                   for gds in self.gdss], 
                  index=self.conds, columns=self.pids)
    get_eigvts = get_eigvs  # for backward compatibility; deprecation warning
    
                  
    def integrate(self, print_cond=True, **kwargs):
        """Calling signature is the same as Geodesic.integrate, pasted below:
        """
        for cond, gds in self.gdss.items():
            if print_cond:
                print cond
            gds.integrate(**kwargs)
    integrate.__doc__ += Geodesic.integrate.__doc__
    ...
    #def get_subset(self, idxs=None):
    ##    if idxs is not None:
    #       conds = list(itertools.product(idxs, (True, False)))
    #    return 
    
    def plot_trajs(self, vartype, log10=None, nrow=None, ncol=None, 
                   subplots_adjust=None, figtitle='', figsize=None,
                   show=True, filepath='', axs=None, **kwargs):
        if axs is None:
            if nrow is None and ncol is None:
        ...
        for ax, gds, cond in zip(axs, self.gdss, self.conds):
            ...
            if (log10 is None and vartype in ['_s', 's']) or log10 is True:
                traj = np.log10(traj)
                
            traj.plot(ax=ax, title=None, **kwargs)  # hacking: title should be cond
    ...
    def plot_eigvts(self, idx_eigv=-1, nrow=None, ncol=None, cmapname=None, 
                    subplots_adjust=None, figtitle='', figsize=None,
                    show=True, filepath='', axs=None, **kwargs):
        """FIXME **: What's this function doing?? Plus I've removed p0 
        as an attribute so self.p0 below won't work.
        """
        if axs is None:
            if nrow is None and ncol is None:
                nrow, ncol = int(np.ceil(len(self.conds)/2)), 2 
    ...
    def plot(self, vartypes=['_p','_y','_s','eigvt'], 
             figsize=(18,10), subplots_adjust={'wspace':0.8,'hspace':0.8}, 
             figtitle='', show=True, filepath='',
             **kwargs):
        """
        """
        _vartype2ylabel = dict(_p=r'$p$', _y=r'$y$', _s='$\log_{10}\sigma$',
                               eigvt='eigenvector')
        _vartype2cmapname = dict(_p='jet', _y='standard', _s='brg', eigvt='jet',
                                 p='jet', y='standard', s='brg')
        ylabels = butil.get_values(_vartype2ylabel, vartypes)
        cmapnames = butil.get_values(_vartype2cmapname, vartypes)
        
        n = len(vartypes)
        nrow = int(np.ceil(len(self.conds)/2))
        ncol = 2 * n
        axidxs = list(itertools.product(range(nrow), range(ncol)))[:len(self.gdss)*n]
        axs = plotutil.get_axs(nrow, ncol, axidxs, figsize=figsize,
                               subplots_adjust=subplots_adjust)
        
        for idx, (vartype, ylabel, cmapname) in enumerate(zip(vartypes, ylabels, cmapnames)):
            axs_vartype = np.reshape(axs, (nrow, ncol))[:,idx::n].ravel()
            try:
                self.plot_trajs(vartype, axs=axs_vartype, colorscheme=cmapname,
                                xylabels=['',ylabel])
            except AttributeError:    
                getattr(self, 'plot_'+vartype+'s')(axs=axs_vartype, 
                                                   cmapname=cmapname,
                                                   xylabels=['',ylabel])
    ...
    def get_widths(self, kind='geodesic'):
        """
        Input:
            kind: 'geodesic' or 'euclidean'
        """
        pass
    ...
    def get_geodesic(self, cond, reset=True, **kwargs):
        """For re-integration.
        """
        gds = self.gdss[cond]
        if reset:
            gds = gds.reset(**kwargs)
        return gds
