...
hd_mm11Alin = hd_mm11.get_subdiagram(root='Alin')
hd_mm11Plin = hd_mm11.get_subdiagram(root='Plin')
hd = hasse.get_product([hd_mm11Alin, hd_mm11Plin], f_nodeattr=lambda nodeattrs: 
                       {'ratelaws': [d['ratelaw'] for d in nodeattrs]})

nodeids1 = hd_mm11.get_subdiagram(root='fAlin').nodeids
nodeids2 = hd_mm11.get_subdiagram(root='fPlin').nodeids
nodeids = butil.get_product(nodeids1, hd_mm11Plin.nodeids) +\
          butil.get_product(hd_mm11Alin.nodeids, nodeids2)
#nodeid2color = dict.fromkeys(nodeids, 'light red')
#hd.draw(width=500, height=500, rank2size=dict.fromkeys(hd.order.keys(), (1,1)), 
#        nodeid2color=nodeid2color, filepath='hd.pdf')

hd = hd.get_subdiagram(nodeids=nodeids)        
nodeids_remove = hd.get_subdiagram(root=('fAlin','fPlin')).nodeids

#hd.rm_node(('fAlin','fPlin'))
...
def get_pred(node):
    nodeattr = node[1]
    net = nodeattr['net']
    expts = experiments.get_experiments(net.Jids[0], uids=['C1','C2'], 
                                        us=butil.get_product(*[[1,2,3]]*2))
    pred = net.get_predict(expts, tol=1e-13)
    nodeattr['pred'] = pred
    
    pred = predict.str2predict(s='-'.join(net.ratelaws), pids=net.pids, 
                               uids=['X'], us=np.linspace(0.5,5,10).reshape(10,1), 
                               c={'C1':2, 'C2':0.5})
...
hd.draw(width=500, height=500, rank2size=dict.fromkeys(hd.order.keys(), (1,1)), 
        nodeid2color=dict.fromkeys(nodeids_remove, 'light red'), filepath='hd2.pdf')
