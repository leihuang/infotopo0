...
from infotopo.models.rxnnet import trajectory, structure, mca, algebra, experiments
reload(trajectory)
reload(mca)
reload(algebra)
reload(experiments)
...
class Network(object, Network0):
    ...
    @property
    def p(self):
        return self.optvars.apply(lambda var: var.value)
    ...
    @property
    def p0(self):
        return self.optvars.apply(lambda var: var.val0)
    ...
    @property
    def params(self):
        return Series(OD(self.parameters.items()), dtype=object)
    
    def to_p(self, p):
        return Series(p, self.pids)
    ...
    @property
    def convars(self):
        return Series(OD(self.constantVars.items()), dtype=object)
        
    @property
    def convarvals(self):
        return self.convars.apply(lambda var: var.value)
    ...
    def replace_ratelaw(self, rxnid, ratelaw, p):
        """Replace the ratelaw of a reaction (surgery). A more general version
        of reduce. 
        
        Input:
            p: the same requirement as in add_reaction
        """
        net2 = self.copy()
        
        rxn = net2.rxns[rxnid]
        
        pids0 = exprmanip.extract_vars(rxn.kineticLaw).intersection(net2.pids)
        for pid0 in pids0:
            # If the pid only appears in the ratelaw or 
            # the ratevar assignment rule then remove it.
            uses = net2.get_uses(pid0) 
            locs = butil.flatten([use.keys() for use in uses.values()])
            if len(set(locs) - {rxnid, 'v_'+rxnid}) == 0:
                net2.remove_component(pid0)
        
        rxn.kineticLaw = ratelaw
        
        # The following chunk is copied from add_reaction
        for pid, pinfo in p.items():
            if isinstance(pinfo, tuple):
                pval, is_optimizable = pinfo
            else:
                pval, is_optimizable = pinfo, True
            if pid in net2.parameters.keys():
                if net2.parameters.get(pid).value != pval:
                    raise ValueError("Value of parameter %s in reaction %s different."%\
                                     (pid, rxnid)) 
            else:
                net2.add_parameter(pid, pval, is_optimizable=is_optimizable)
        
        rxn.parameters = exprmanip.extract_vars(ratelaw).difference(set(net2.xids))
        
        if 'v_'+rxnid in net2.asgrules:
            net2.assignmentRules.set('v_'+rxnid, ratelaw)
            
        # Reorder the pids so that the original order is preserved
        # assuming the pids of the operated reaction are together
        idxs = [self.pids.index(pid0) for pid0 in pids0]
        pids2 = self.pids[:min(idxs)] + p.keys() + self.pids[max(idxs):]
        net2 = net2.reorder_parameters(pids2)
    ...
    def add_ratevars(self):
        """Add or update rate variables. Update is needed if ratelaws of
    ...
    def print_details(self):
        """
        """
        convarvals = self.convarvals[[varid for varid in self.convarids if
                                      varid not in self.pids]]
        #details = OD(zip(['N', 'ratelaws', 'p', 'x0', 'convarvals'], 
        #                 [self.N, self.ratelaws, self.p, self.x0, convarvals]))
  
        for attr in [self.N, self.ratelaws, self.p, self.x0, convarvals]:
            print attr, '\n'
        '''
        print "Species:"
    ...
    def perturb(self, condition):
        ...
        if condition == ():  # wildtype  
            ...
            for perturbation in condition:
                ...
                if mode == '=':
                    net.set_var_ic(varid, strength)  # need to verify...
            net.compile()
            return net
    ...
    def get_predict(self, expts, p0=None, name='', **kwargs): 
        ...
        cond2net = OD([(item[0],item2[0]) for item, item2 in zip(items,items2)])
        ...
        pred = predict.Predict(f=f, Df=Df, p0=net0.p, name=name, pids=net0.pids, 
                               yids=expts.yids, expts=expts, cond2net=cond2net)
        return pred
...
###############################################################################
    ...
    def get_dxdt(self, t=None, x=None, to_ser=True):
        """
        ...
        # >>> print net.constantVarValues
        #print self.convarvals
        #print Series(self.constantVarValues, self.constantVars.keys())
        dxdt = self.res_function(t, x, np.zeros(len(x)), self.constantVarValues)
        #dxdt = self.res_function(t, x, np.zeros(len(x)), self.constantVarValues)
        if to_ser:
...
###############################################################################
    ...
    def get_J(self, to_ser=False, **kwargs):
    ...
    def update(self, p=None, t=None, x=None, t_x=None, **varmap):
        ...
        if varmap:
            for varid, varval in varmap.items():
                ...
                # before it was set_var_val; upon a closer look at the codes 
                # of set_var_val and set_var_ic, set_var_ic is chosen;
                # set_var_ic updates the constantVarValues, which is used
                # in many computations (say, integration)
        ...
        # Rewriting the above chunk...
        # The problem is: if net is at time t and I update its p, 
        # and then I want to update net to time t (or get the x of time t), 
        # it would erroneously not update at all (or output the wrong x(t) of
        # the previous p). To fix this but still avoid redundant computation, 
        # I can have a flag like state_up_to_date that is set False whenever
        # p is changed and set True whenever a new computation has been done
        # for the latest p. 
        ## What's the gain of efficiency? Need to time it! So far all based on
        # wishful thinking. Woeful. 
        ## Write some tests!
        
        if t is not None:
            self.get_traj(times=[t])
