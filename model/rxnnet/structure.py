...
def get_stoich_mat(net=None, rxnid2stoich=None, only_dynvar=True, 
    ...
    if net:
        try:
            ...
            if only_dynvar:
                rowvarids = net.dynvarids
            else:
                rowvarids = net.spids
            N = Matrix(np.zeros((len(rowvarids), len(net.rxnids))),
                       rowvarids=rowvarids, colvarids=net.rxnids)
...
def get_pool_mul_mat(net):
    ...
    try:
        P = net.pool_mul_mat
        N = net.N
        if (P is None and N.rank == N.nrow) or\
        ...
        if N.rank == N.nrow:
            ...
            P = Matrix(vecs, 
                       rowvarids=['Pool%d'%idx for idx in range(1, len(vecs)+1)],
                       colvarids=N.rowvarids)
            # clean things up a bit...
            P = P.fliplr().rref().fliplr()
        net.pool_mul_mat = P
...
def get_ss_flux_mat(net):
    ...
    try:
        K = net.ss_flux_mat
        N = net.N
        if (K is None and N.rank == N.ncol) or\
        ...
        if N.rank == N.ncol:
            ...
            K = Matrix(np.transpose(vecs), rowvarids=N.colvarids, 
                       colvarids=['Flux%d'%idx for idx in range(1, len(vecs)+1)])        
        net.ss_flux_mat = K
        
        return K


def get_ddynvarids(net):
    P = net.P
    ...
    # preserve the order
    # ddynvarids = [varid for varid in net.dynvarids if varid in ddynvarids]
    return ddynvarids


def get_idynvarids(net):
    ddynvarids = net.ddynvarids
    idynvarids = [_ for _ in net.dynvarids if _ not in ddynvarids]
    return idynvarids 
...
def get_reduced_link_mat(net):
    ...
    #for i, ddynvarid in enumerate(ddynvarids):
        ...
        # each row is replaced by (-row/multiplicity(ddynvarid))
    #    L0[i] /= -P[i, ddynvarid]
    return L0
...
def reorder_dynvarids(net):
    ...
    """
    idynvarids, ddynvarids = net.idynvarids, net.ddynvarids
    net2 = net.reorder_species(idynvarids+ddynvarids) 
    return net2
